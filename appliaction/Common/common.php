...
function  SendMail($address,$title,$message) {
	 ...
	 //$smtp->debug=true;
	 $send = $smtp->sendmail($address, $smtpuser, $title, $message, $mailtype);
...
function checktplrefresh($maintpl, $cachefile, $timecompare = 1) {
	 ...
	 $timestamp = NOW_TIME;
	 if(empty($timecompare) || $tplrefresh === FALSE || !file_exists(DOC_ROOT.$cachefile) ||  @filemtime(DOC_ROOT.$cachefile) - $timestamp > $timecompare) {
		  libfile('Template');
...
function template($file = '', $module = '') {
    ...
    $module = (empty($module)) ? GROUP_NAME : $module;
    ...
    $filebak = $file;
    $tplfile = strtolower($tpldir.'/'.$file.$suffix);
    ...
    /* 检查模板缓存 */
    checktplrefresh($tplfile, $cachefile, @filemtime(DOC_ROOT.$cachefile));
    return $cachefile;
}
...
//获取haha_secwk会员表信息
function getHahaSecwkMenberfield($id,$field){
//    $User = M("User",DB_PREFIX,DB_URL);
//    $where['id']=$id;
//    $field=$field;
//    $User->where($where)->getField($field);
    return  "111";
}
 ...
 */
 ...
 function sendPublicMsgByMobile($mobileNo, $type=1, $msg='',$message='')
    {
        $post_data = array();
		...
		if($type == 1) {
			// 发送验证码 1
			$message="亲爱的【威客安全】用户，您的自定义需求，管理员已经处理，我们会尽快与你取得联系。";
		} else if($type == 2) {
			// 通过 2
			$message="亲爱的【威客安全】用户，您申请的众测".$msg."已经审核通过，我们会尽快与你取得联系。";
		} else if($type == 3) {
//			// 驳回 3
			$message="亲爱的【威客安全】用户~非常抱歉，您发布的众测项目被驳回，驳回原因请您登陆www.secwk.com进行查看。";
		} else if($type == 4) {
			// 到期 4
			$message="亲爱的【威客安全】用户，您发布的".$msg."众测项目已经结束，您的漏洞修复完成了吗？白帽子希望你们更安全。";
		} else if($type == 5) {
//			// 漏洞提交 5
			$message="亲爱的【威客安全】用户，您发布的".$msg."众测项目有新的漏洞提交了，赶快登陆www.secwk.com进行确认哦~白帽子提醒您，请尽快修复漏洞。";
		} else if($type == 6) {
			// 授权成功 6
			$message="亲爱的【威客安全】用户，您的授权已经成功，请尽快登陆www.secwk.com进行查看。";
		} else if($type == 7) {
			// 认证成功 7
			$message="亲爱的【威客安全】用户，您申请的白帽子/销售认证已经成功，请尽快登陆www.secwk.com进行查看。";
		} else if($type == 8) {
			// 认证失败 8
			$message="亲爱的【威客安全】用户~非常抱歉~您提交的白帽子/销售认证失败，失败原因请您登陆www.secwk.com进行查看。";
		} else if($type == 9) {
			// 充值成功 9
			$message="亲爱的【威客安全】用户，用户名为".$msg['username']."成功充值".$msg['money']."元，感谢您对我们的支持。";
		} else if($type == 10) {
			// 提现成功 10
			$message="亲爱的【威客安全】用户，用户名为".$msg['username']."成功提现".$msg['money']."元，请您尽快查看资金是否到账。感谢您对我们的支持。";
		}
	...
	function  getData($key){
	 ...
	 function sendMsgByEmail($email, $msg)
	{
		vendor(email);
		//******************** 配置信息 ********************************
		$smtpserver = "smtp.vip.163.com";//SMTP服务器
		$smtpserverport =25;//SMTP服务器端口
		$smtpusermail = "secwk@vip.163.com";//SMTP服务器的用户邮箱
		$smtpemailto = $email;//发送给谁
		$smtpuser = "secwk";//SMTP服务器的用户帐号
		$smtppass = "UJqghHzL%jaP4Hpi";//SMTP服务器的用户密码
		$mailtype = "HTML";//邮件格式（HTML/TXT）,TXT为文本邮件
		//************************ 配置信息 ****************************

		$mailtitle = "威客安全注册激活邮件";//邮件主题
		$mailcontent = "验证码为：".$msg."，感谢您注册威客安全平台，请填写完成验证";//邮件内容
		$smtp = new smtp($smtpserver,$smtpserverport,true,$smtpuser,$smtppass);//这里面的一个true是表示使用身份验证,否则不使用身份验证.
		$smtp->debug = false;//是否显示发送的调试信息
		$state = $smtp->sendmail($smtpemailto, $smtpusermail, $mailtitle, $mailcontent, $mailtype);
		if($state==""){
			$message="验证码为：".$msg."，感谢您注册威客安全平台，请填写完成验证";
			$emailmodel = array();
			$emailmodel["title"] ="威客安全注册激活邮件";
			$emailmodel["email"] =$email;
			$emailmodel["content"] =$message;
			$emailmodel["issend"] ='0';
			$emailmodel["create_time"] =time();
			$re = D('email_temp_instant')->add($emailmodel);
			exit();
		}
		$message="验证码为：".$msg."，感谢您注册威客安全平台，请填写完成验证";
		$emailmodel = array();
