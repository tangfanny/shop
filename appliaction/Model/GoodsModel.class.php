...
class GoodsModel extends SystemModel {
    ...
    public function update_goods_xl($goods_id, $num, $shop_number) {
        $param["id"] = $goods_id;
        $data["buy_num"] = $num + $shop_number;
        return $this->where($param)->data($data)->save();
    }

    public function select_googds_by_id($goods_id){
        $param["id"] = $goods_id;
    ...
    public function detail($id, $product_id = 0, $field = TRUE, $extra = TRUE) {
        ...
        $goods_info['url'] = U('Index/show', array('id' => $id));
        //取会员信息
//		$user_group = model('user')->getFieldById(''.is_login().'','group_id');
	...
	public function setDecNumber($goods_id, $product_id, $goods_number) {
		...
		$this->_db->where($sqlmap)->setDec('goods_number', $goods_number);
		return TRUE;
	}
	...
	public function update(){
		...
		//详细描述
		$_POST['descript'] = $PD['descript']?$PD['descript']:'';
		//图册列表
		$_POST['list_img'] = '';
		if($PD['goodsphoto']) {
			$_POST['list_img'] = arr2str($PD['goodsphoto']);
		}
		$_POST['explain_extension'] = $PD['explain_extension']?$PD['explain_extension']:'';
		//其它属性
		$_POST['status_ext'] = '';
		...
		$_POST['is_recommend'] = !empty($PD['is_sales']) ? 1 : 0;
		$_POST['is_new'] = !empty($PD['is_new']) ? 1 : 0;
		$_POST["recommend_app"]=!empty($PD['recommend_app']) ? 1 : 0;
		$_POST["selected"]=!empty($PD['selected']) ? 1 : 0;
		if(!$this->create()){
		...
		if(isset($_POST['id']) && $_POST['id'] > 0) {
			...
			$_POST['id'] = $this->getrandoms();
			$this->add($_POST);
		}
		$this->create_goods_vendor_service();
		$this->create_product_info();
		$this->create_cert_list(); //添加证书列表
		/* 属性数据组织 */
	...
	//添加证书列表
	public function create_cert_list(){
		$cert_list = M('certificate_list');
		$cid = $_POST['certificate_id'];
		$cert_list->where(array('goods_id'=>$_POST['id']))->delete();
		$list = array();
		foreach ($cid as $key => $value) {
			$list[$key]['goods_id'] = $_POST['id'];
			$list[$key]['brand_id'] = $_POST['brand_id'];
			$list[$key]['certificate_id'] = $value;
		}
		$cert_list->addAll($list);
	}
	...
	public function deleteGood($id){
		if(!empty($id)){
			...
			M('goods_rank_sum')->where(array('gid' => array('IN', $id)))->delete();
			return true;
		}else{
			return false;
		}
	...
	private function create_goods_vendor_service(){
		$vendorid=$_POST["server"];
		$vendoremoney = $_POST["money"];
		$vendorname=$_POST["vendor_yeas"];
		$product_id= $_POST["model1"];
		$goods_id = $_POST["id"];
		$vendor_sort=$_POST["vendor_sort"];
		$db = model("vendor_service_list");
		if($product_id==-1){
		...
		if($product_id!=0){
			...
			$db->where($where)->delete();
			$adata=array();
			for($i=0;$i<count($vendoremoney);$i++){
				if($vendoremoney[$i]!=0){
					$item = array();
					$item["goods_id"]=$goods_id;
					$item["products_id"]=$product_id;
					$item["vendor_name"]=$vendorname[$i];
					$item["vendor_id"]=$vendorid;
					$item["vendor_money"] = $vendoremoney[$i];
					$item["sort"]=$vendor_sort[$i];
	...
	private function carete_goods_attr_spec(){
		...
		if($products_id!=0){
			$db->where("goods_id=".$goods_id." and products_id=".$products_id)->delete();
...
//		属性处理
	...
	private function buildMap($params = array()) {
		...
		if((int)$params['id']) {
			...
			if (!$cat_ids) $sqlmap['cat_ids'] = (int)$params['id'];
			$_dchildids = array((int)$params['id']);
			$cat_ids = array_merge($cat_ids,$_dchildids);
			if (count($cat_ids) > 0) $sqlmap['cat_ids'] = array('IN',$cat_ids);
		}
		/* 筛选品牌 */
	 ...
	 * @date
	 */
	public function get_goods_name($name){
		$param["name"]=$name;
		return $this->where($param)->find();
	}
    ...
    public function update_goods_buy_num_by_id($id, $data) {
        $param["id"] = $id;
        return $this->where($param)->data($data)->save();
    }
