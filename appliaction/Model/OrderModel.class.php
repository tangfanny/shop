...
class OrderModel extends SystemModel {
	...
	public function detail($order_sn = '', $field = TRUE, $extra = TRUE) {
        ...
        public function detailleft($order_sn=''){
            if(empty($order_sn)){
                $this->error  = '订单号不能为空';
		return false;
            }
            $sqlmap = array();
		$sqlmap['order_sn'] = $order_sn;
		$rs = $this->join("t_order_staging as sordestag on t_order.id=sordestag.orderid ")
//                        ->field("sordestag.id,sordestag.money,payable_amount,payable_freight,taxes,insured,discount,coupons,integral")
                        ->order(" sordestag.staging_num asc ")
                        ->where($sqlmap)
                        ->select();
		if(!$rs) {
			$this->error = '订单号不存在';
    ...
    public function updateorderpay($id,$order){
        $data["id"]=$id;
        $flag = $this->where($data)->data($order)->save();
        return $flag;
    }
    /**
     * 根据订单编号查询订单
