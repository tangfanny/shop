...
class SkillController extends AdminBaseController {
    ...
    public function add(){
        ...
        if(IS_POST) {
            ...
            include $this->admin_tpl("skill_add");
        }
    ...
    public function edit(){
        ...
        $validform = TRUE;
        if(IS_POST){
            if($_POST['old_pid']==0 && $_POST['old_pid']!=$_POST['parent_id'])showmessage('不允许移动主技能');
            $ptree=$this->findFather($_POST['parent_id']);
            if(in_array($_POST['old_pid'],$ptree)){
                showmessage("父技能不能移动到子技能",NULL,0);
            }
            if($this->db->create()){
                $this->db->save();
                $this->db->build_cache();
...
//			var_dump($tree);
            include $this->admin_tpl("skill_edit");
        }
    }
    /**
     * 列表
    ...
    public function lists(){
            include $this->admin_tpl('skill_lists');
    }
    /**
     * 列表
     */
    public function skill_child(){
