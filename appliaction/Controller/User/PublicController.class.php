	...
	public function login() {
		if (IS_POST) {
			...
			if(strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger') != false ){
			    ...
			    $options['encodingAesKey']=md5(C('token'));
	...
	public function repwd(){
		if(IS_POST){
			...
			if($email){
				...
				$url = getconfig('site_companyurl').U('User/Public/setrepwd?repwd_key='.$repwd_key.'');
				//模板
				$msg_template = model('MsgTemplate')->where('id=4')->find();
				$title = $msg_template['title'];
				$content = $msg_template['content'];
				$subject = str_replace(array('{$site_name}'), array($site_name), $title);
				$body = str_replace(array('{$user_name}','{$site_name}','{$url}'), array($user_name,$user_name,$url), $content);
				$r = SendMail($email['email'],$subject,$body);
