...
//
    ...
    private function push_coupons($member_id, $push_num, $coupons_id) {
        ...
        if($coupons_count<$push_num_count){
            showmessage("发放失败，优惠券数量小于发放人数！");
            exit();
        }        
        ...
        foreach ($member_id_arr as $k => $v) {
            ...
            $where["id"]=$user_id;
            $field="nikename";
            //$user_name = getMemberfield($v['user_id'],'nikename');老版本查询用户表，新版本采用haha_secwk数据
            $user_name = $User->where($where)->getField($field);
            //发送代金券
