	...
	<div class="goods mt10">
		<div class="guanli">
			<form action="<?php echo U('Member/lists'); ?>" method='post'>
				...
				<select name='name' id='group_combbox' class="easyui-combobox" data-options="editable:false,panelHeight:'auto'" style="height: 26px;" prompt="请选择"></select>
				<span style="margin-right: 10px;margin-left: 10px;">搜索</span>
				<input id="keyword" class="easyui-textbox" name="keyword" style="width:210px;height: 26px;" prompt="输入手机/邮箱/均可搜索">
...
<script type="text/javascript">
	...
	var orderurl = '<?php echo U('Goods/AdminOrder/lists')?>';
        var invoice = '<?php echo U('MemberInvoice/lists')?>';
...
//			],
			...
			columns:[[
				{field:'nikename',title:'会员名',fixed:true,align:'center',width:'10%',sortable:true},
				{field:'role',title:'会员角色',fixed:true,width:'10%',align:'center'},
				{field:'name',title:'会员等级',fixed:true,width:'12%',align:'center',
					formatter:function(value,row,index){
						if(value!=null && value!=0 && row.is_saler_auth !=0){
							return value;
						}else{
							return "-";
						}
				...
				{field:'mobile',title:'手机号',fixed:true,width:'13%',align:'center'},
				{field:'email',title:'电子邮箱',align:'center',width:'13%',align:'center'},
...
//				{field:'exp',title:'经验',width:'6%',align:'center',sortable:true},
				{field:'create_time',title:'注册时间',width:'16%',align:'center',sortable:true,
					formatter:function(value,row,index){
				...
				{field:'userID',title:'操作',width:'26%',align:'center',halign:'center',
					formatter:function(value,row,index){
						var sp_txt="&nbsp;&nbsp;&nbsp;&nbsp;";
							return '<a href="'+addressurl+'&user_id='+row.id+'">地址</a>'+sp_txt+
							'<a  href="'+orderurl+'&user_id='+row.id+'">订单</a>'+sp_txt+
                                                        '<a  href="'+invoice+'&user_id='+row.id+'">发票</a>'+sp_txt+
...
//							'<a  href="'+editurl+'&id='+row.id+'">编辑</a>'+sp_txt+
							...
							'<a  href="'+detailurl+'&id='+row.id+'">查看详情</a>';
//							'<a onclick="dom.datagrid(\'clearSelections\').datagrid(\'clearChecked\').datagrid(\'checkRow\','+index+');$(\'#delrows\').trigger(\'click\')" href="#">删除</a>';
		...
		//查询参数直接添加在queryParams中
			...
			queryParams.name = $('#group_combbox').combobox('getValue');
