...
class GoodsController extends AdminBaseController {
    ...
    public function _initialize() {
        ...
        //标签列表
        $this->label = model('goods_label')->where('status = 1')->order('sort ASC')->select();
        $this->db = model('Goods');
        $this->model = model('Model')->order('sort ASC,id DESC')->select();

        $this->server = model("vendor_service");
...
//                        exit;
    ...
    public function add() {
        ...
        if (IS_POST) {
            $id = $this->db->update();
            if ($id) {
                showmessage('提交成功', U('edit?addflag=' . $addflag . "&id=" . $id), 1);
            } else {
		...
		if(IS_POST){
			...
			if($result){
				if($_GET["addflag"]!='add'){
					...
					showmessage('提交成功',U('edit?addflag='.$_GET["addflag"]."&id=".$id),1);
				}
			}else{
            ...
            $join_brand_vendor = "LEFT JOIN __BRAND__ as b
                                            ON a.brand_id=b.id";
            $server = $this->server->alias("a")->join($join_brand_vendor)->field("b.name,a.value,a.id,a.type,a.sort,a.status")->select();
            $category = model('Category')->getInfoByIds($_list['cat_ids']);
            $validform = TRUE;
            $dialog = TRUE;
            $addflag = $_GET["addflag"];
            $posturl = U('edit?addflag=' . $_GET["addflag"] . "&id=" . $id);

            //反序列化字段中的数组
            foreach ($product as $k => $v) {
                $product[$k]['spec_array'] = unserialize($v['spec_array']);
                //查询关联服务年限
                $productpid = M('GoodsProducts')->where('pid = ' . $v['id'])->select();
                $product[$k]['productpidlist'] = $productpid;
            }
    ...
    public function getserver() {
        $con['id'] = $_POST['serverid'];
        $data = $this->server->field('value')->where($con)->find();
        $where['vendor_id'] = $con['id'];
        $where['products_id'] = $_POST["product_id"];
        $data2 = model("vendor_service_list")->where($where)->select();
        ...
        foreach ($data as $key => $value) {
            ...
            foreach ($data2 as $k => $v) {
                if ($data2[$k]['sort'] == $key + 1) {
                    $b[$key]['val'] = $data2[$k]['vendor_money'];
                }
            }
        }
        echo json_encode($b);
...
;
    ...
    public function goods_add_value() {
        ...
        if (empty($new_value))
            $this->error('请填写要添加的属性值');
        if ($data['id'] < 1)
            $this->error('该规格不存在或规格ID有误');
    ...
    public function update_goods_rand() {
        $goods_array = $this->db->get_goods_array();
        foreach ($goods_array as $k => $v) {
            $param["buy_num"] = rand(1, 100);
//                 echo rand(1, 100),"<br>";
            $this->db->update_goods_buy_num_by_id($v["id"], $param);
        }
    }
