...
class GoodsMessageController extends AdminBaseController {
	...
	public function send_mail(){
		...
		if($id){
			$_list = $db->where('id='.$id.' AND !ISNULL(email)')->select();
		}else{
		...
		$send_num = 0;
		$err_num = 0;
		$msg_tpl = model('Msg_template')->where('id=1')->Find();
		foreach($_list as $k=>$v){
			$user_name = getMemberfield($v['user_id'],'username');
			$user_name = $user_name ? $user_name : '游客' ; 
			
			$subject = str_replace(array('{$user_name}','{$goods_name}'), array($user_name,$v['goods_name']), $msg_tpl['title']);
			$body = str_replace(array('{$user_name}','{$goods_name}'), array($user_name,$v['goods_name']), $msg_tpl["content"]);
