...
class EditorController extends AdminBaseController {
    ...
    public function upload() {
        ...
        $ext_arr = array(
            'image' => array('gif', 'jpg', 'jpeg', 'png', 'bmp','PDF','pdf','rar','zip','doc','docx','xls', 'xlsx'),
            'flash' => array('swf', 'flv'),
        ...
        if(IMG_INTREFACE == 1){
            ...
            if ($_FILES[$file_key]['error']==0 && in_array($extension, $allowExts)) {
                ...
                $filePath = $file_path;
                // 上传到七牛后保存的文件名
                if(in_array($extension,$doc_exten)){
                    $key = time().rand(100000,999999).'.'.$extension;
                }else{
                ...
                if ($err !== null) {
                    ...
                    $this->ajaxReturn(array('error' => 0, 'url' =>$ret['key'] ,'title'=>$extension));
                    exit();
                }
            }else{
    ...
    public function file_manage() {
        ...
        $ext_arr = array('gif', 'jpg', 'jpeg', 'png', 'bmp','pdf','PDF');
        //目录名
