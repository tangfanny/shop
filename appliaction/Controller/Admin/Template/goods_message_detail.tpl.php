...
<script>
	//批量发送邮件
	function send_mail(goods_id,product_id,user_id) {
		$.messager.confirm("发送到货邮件通知", '发送邮件可能比较费时,如果邮件较多,请使用专业软件发送!', function (r) {
			if (r) {
				var url = "<?php echo U('Goods/GoodsMessage/send_mail') ?>";
				$.messager.progress();
				$.getJSON(url, {"goods_id": goods_id,"product_id": product_id,"id":user_id}, function(data) {
...
<!--表格js-->
	...
	$(function(){	
		dom.datagrid({   
			...
			columns:[[
				{field:'user_name',title:'用户名',width:'15%',halign:'center',align:'center'},
				{field:'email',title:'用户Email',width:'20%',align:'center',sortable:true},
				{field:'mobile',title:'手机号',width:'15%',align:'center',sortable:true},
				...
				{field:'dateline',title:'用户登记时间',width:'10%',halign:'center',align:'center',sortable:true,
					formatter:function(value,row,index){  
				...
				{field:'none',title:'操作',width:'20%',halign:'center',align:'center',
					formatter:function(value,row,index){
						var sendemailthtml = '<a href="javascript:" onclick="send_mail('+row.goods_id+','+row.product_id+','+row.user_id+');">发送到货邮件</a>&nbsp;&nbsp;&nbsp;&nbsp;';
