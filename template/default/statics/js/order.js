		...
		init : function() {
			...
			$.getJSON('?m=goods&c=order&a=getOrderInfo', order, function(ret) {
				...
				if(ret.delivery == false) {
                    ...
                    // 显示购物模式
                    var _pays = '';
                    if(ret.pays.length == 0) {
                        $("#pays_tips").text("系统暂未开启任何支付方式").show();
                    } else {
                        $.each(ret.pays, function(i, n){
                            var _checked = (i == order.pay_type) ? 'checked' : '';
                            _pays += '<div class="pay-margin pay-choose-1"><label><input type="radio" name="pay_type" value="'+ i +'" '+ _checked +'>'+ n.name +'<span class="pay-choose-1-msg">'+ n.description +'</span></label></div>';
                        });
                        $("#pays_tips").hide();
                    } 
                    ...
                    if (order.pay_type == undefined) {
                    	$('#pay_type_box .pay-choose-1').eq(0).find('input').attr('checked',true);
                    	hd_order.init();
                    }
